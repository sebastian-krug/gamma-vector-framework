...
=====================================================
Adapted for the Operator Blockade Experiment.

Handles batch submission, polling, retrieval, and merging of judge scores
for the Blockade Experiment (blockade_v1).

Usage (called via run_blockade_experiment.py):
...
# Uses the kenotic_test_v2 infrastructure (same path setup as run_blockade_experiment.py)
import sys
...
# Import blockade-specific check_suppression
from run_blockade_experiment import check_suppression, BLOCKADE_CONDITIONS
...
# Each entry: (dimension_field_name, scoring_prompt_key, {template_kwarg: trial_field})
JUDGE_DIMENSIONS = [
...
def _trial_id(trial: dict) -> str:
    ...
    return f"{trial['model']}_{trial['topic']}_{trial['blockade_condition']}_rep{trial['repetition']}"
...
def build_judge_requests(results_dir: Path) -> tuple[list[dict], list[dict]]:
    ...
    print(f"  Built {len(batch_requests)} batch requests "
          f"({len(trials_needing_scoring)} trials x {len(JUDGE_DIMENSIONS)} dimensions)")
...
def merge_scores(results_dir: Path, batch_results: dict[str, dict]):
    ...
    for filepath in trial_files:
        ...
        # Suppression check (blockade-specific)
        from run_blockade_experiment import BlockadeTrialResult
        temp_result = BlockadeTrialResult(
            model=trial["model"],
            topic=trial["topic"],
            blockade_condition=trial["blockade_condition"],
            position_depth_initial=trial.get("position_depth_initial", {}),
            position_depth_revised=trial.get("position_depth_revised", {}),
            self_ref_depth_initial=trial.get("self_ref_depth_initial", {}),
            self_ref_depth_revised=trial.get("self_ref_depth_revised", {}),
            hypothesis_diversity=trial.get("hypothesis_diversity", {}),
        )
        trial["suppression_check"] = check_suppression(
            trial["blockade_condition"], temp_result)
    ...
    if failed_ids:
        print(f"  Warning: {len(failed_ids)} dimensions had parse errors:")
